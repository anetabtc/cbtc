import { initLucidWithoutWallet } from "./utils/lucid";
...
export const request = async (lucid, amount, ada_addr, btc_addr) => {
    ...
    const result = await user_request.submit(lucid, bridgeAmount, myAddress, btcAddress, deployments.scripts.guardianValidator);
    console.log(result);
};
...
export const sendBitcoin = async (recieverAddress, amountToSend) => {
...
async function execute_mint(lucid) {
...
async function update_redeem_queue() {
    ...
    for (let i in txs) {
        let tx_id = txs[i]["tx_hash"]; // "ffa3f3263803c64ea350c2eabd065072b012c3018951edafd9ee276cb5aa2b0c"
...
function redeem(btc_addr, amount) {
...
async function RedeemAPI(sender_addr, amount, receiver_addr) {
