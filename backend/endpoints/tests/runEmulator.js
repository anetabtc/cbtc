import { Emulator, generatePrivateKey, generateSeedPhrase, Lucid, } from "lucid-cardano";
...
const generateAccountPrivateKey = async (assets) => {
...
const generateAccountSeedPhrase = async (assets) => {
...
export const run = async () => {
    ...
    console.log("[INFO] New UTXO at multisigValidator: ", await lucid.utxosAt(lucid.utils.validatorToAddress(scripts.multiSigValidator)));
    ...
    const resultSubmit = await user_request.submit(lucid, bridgeAmount, myAddress, "15U6C9gZs5G3i11gTfmhqCaKK6V7bqGdmi", scripts.guardianValidator);
    emulator.awaitBlock(4);
    console.log("[INFO] Endpoint result (user_request.submit): ", resultSubmit);
    console.log("[INFO] New UTXO at guardianValidator: ", await lucid.utxosAt(lucid.utils.validatorToAddress(scripts.guardianValidator)));
    ...
    lucid.selectWalletFromPrivateKey(signers.account11.privateKey);
    const witness11 = await multisig_fullfill.signWitness(lucid, fulfillTx.toString());
    lucid.selectWalletFromPrivateKey(signers.account12.privateKey);
    const witness12 = await multisig_fullfill.signWitness(lucid, fulfillTx.toString());
    lucid.selectWalletFromPrivateKey(signers.account13.privateKey);
    const witness13 = await multisig_fullfill.signWitness(lucid, fulfillTx.toString());
    const assembleTx = await multisig_fullfill.assemble(lucid, fulfillTx.toString(), [witness11, witness12, witness13]);
    emulator.awaitBlock(4);
    lucid.selectWalletFromSeed(user.account1.seedPhrase);
    console.log(`[INFO] Received ${bridgeAmount} cBTC at user wallet: `, await lucid.wallet.getUtxos());
    const burnAmount = -5;
    const btcAddress = "15U6C9gZs5G3i11gTfmhqCaKK6V7bqGdmi";
    const resultBurn = await user_burn.submit(lucid, burnAmount, btcAddress, deployments.scripts.cBTCMintingPolicy);
    console.log(resultBurn);
    emulator.awaitBlock(4);
    console.log(`[INFO] Burnt ${burnAmount} cBTC at user wallet: `, await lucid.wallet.getUtxos());
};
