import { Constr, Data, fromText, toUnit } from "lucid-cardano";
import { buildScripts } from "./utils";
export const submit = async (lucid, config) => {
    const walletUtxos = await lucid.wallet.getUtxos();
    const walletTxHash = walletUtxos[0].txHash;
    const walletOutputIndex = walletUtxos[0].outputIndex;
    const scripts = buildScripts(lucid, config.keys[0], walletTxHash, walletOutputIndex);
    const multisigValidatorAddr = lucid.utils.validatorToAddress(scripts.multiSigValidator);
    const multisigPolicyId = lucid.utils.mintingPolicyToId(scripts.multiSigMintingPolicy);
    ...
    const Datum = Data.to(new Constr(0, [config.keys, BigInt(config.requiredCount)]));
